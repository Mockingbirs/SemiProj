...
<script type="text/javascript">
      ...
      function email_change(){
          ...
          function  allcheck(){
        	  ...
        	  ymd = document.getElementById( 'um_ymd' ).value,
        	  tel = document.getElementById( 'um_tel' ).value,
        	  e1 = document.getElementById( 'um_email' ).value,
        	  e2 = document.getElementById( 'um_email2' ).value,
        	  pw = document.getElementById( 'um_password' ).value,
        	  pw2 = document.getElementById( 'join_repw' ).value,
        	  ars = document.getElementById( 'um_ars' ).value,
			...
			if ( ymd == '' || ymd == null ) {alert('정보를 모두 기입해주세요'); return false;}
			...
			if ( ars == '' || ars == null ) {alert('정보를 모두 기입해주세요'); return false;}
 			...
 			var ymdtype = /^[0-9]{8}$/g;		 
 		    if (!ymdtype.test(ymd)) { alert('생년월일이 잘못기입되었습니다.'+ymd); return false;}	
 				
 			var teltype = /^[0-9]{10,11}$/g;  
 			if (!teltype.test(tel)) { alert('휴대폰번호가 잘못기입되었습니다.'+ymd); return false;}
 			
 			var arstype = /^[가-힣]{2,10}$/g;
 			if (!arstype.test(ars)) { alert('이름은 한글만 가능합니다.'+ymd); return false;}
 			
 			var emailtype = /^[^ㄱ-ㅎ|ㅏ-ㅣ|가-힣]{3,20}$/;
 			if (!emailtype.test(e1)||!emailtype.test(e2)) { alert('이메일아이디는 한글사용이 불가능합니다.'+ymd); return false;}
          ...
          $(document).ready(function(){ 
        	  ...
        	  $(":input:radio[name=um_snsch]:checked").val();
		...
		<div class="mjoin">
			<div class="mjoinlayout">
				...
				<div class="m_join_tit">
					<span class="tit">회원정보</span><br> <span class="sub_txt">모든
						항목을 입력해주세요.</span>
				</div>
				<div class="m_form">
					<table>
						...
						<tbody>
								...
								<td colspan="3">
									<div class="input_box">
										<input type="text" id="um_mid" name="um_mid" class="m_join_id"
											value="${member.um_mid}" style="width: 200px" readonly>
										<!--  -->
									...
									<div class="tip_txt if_beside">변경할 수 없습니다.</div>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>
									<div class="input_box">
										<input type="text" id="um_ars" name="um_ars"
											value="${member.um_ars }" readonly class="m_join_name"
											style="width: 120px">
										<div class="tip_txt if_beside">변경할 수 없습니다.</div>
									</div>
								</td>
								...
								<td><div>
										<input type="text" id="um_ymd" name="um_ymd"
											class="m_join_birth" value="${member.um_ymd }" readonly
											style="width: 120px">
										<div class="tip_txt if_beside">변경할 수 없습니다.</div>
									</div></td>
							</tr>
							<tr>
								...
								<td>
									<div>
										...
										<c:if test="${member.um_gender eq 'M'}">checked="checked"</c:if>
											name="um_gender" onclick="return(false);" value="M" class="m_join_sex" ><span
											class="txt">남성</span></label> <label class="label_design"><input
											<c:if test="${member.um_gender eq 'F'}">checked="checked"</c:if>
											type="radio" onclick="return(false);" name="um_gender" value="F"  class="m_join_sex"><span
											class="txt">여성</span></label>
									</div>
									<div class="tip_txt if_beside">변경할 수 없습니다.</div>
								</td>
							...
							<tr>
								...
								<td>
									<div class="input_box">
										<input type="text" name="um_tel" class="m_join_tel" id="um_tel"
											value="${member.um_tel }" style="width: 180px">
									</div>
								...
								<td><label class="label_design"><input type="radio"
								 <c:if test="${member.um_snsch eq '1'}">checked="checked"</c:if>
										name="um_snsch" value=1  ><span class="txt">수신</span></label>
									<label class="label_design"><input type="radio"
									 <c:if test="${member.um_snsch eq '2'}">checked="checked"</c:if>
				...
				<div class="mjoin_btnbox ">
					<ul>
						<li><input type="submit" value="뒤로가기"
							onclick="javascript:window.history.back();" /></li>
						<li><input type="submit" value="메인"  
							onclick="javascript: userInfo.action='toMain.do';" /></li>
						<li><input type="submit" value="수정"
							onclick="return allcheck();" /></li>
					</ul>
				</div>
				<!-- /정보입력 -->
			</div>
