...
//	ArService arService;
	...
	@RequestMapping("/insertBAsk2.zo") // 4점대 버전 이후부터 사용 가능한 기능.
	public String insertBAsk(ProductVO product, AskVO ask, BrandMemberVO brand, Model model) {
		//System.out.println("글 등록 화면 처리");
		
		int bm_no = Integer.parseInt(product.getBm_no());
		
		System.out.println("bm_no =" + bm_no);
		
		
		model.addAttribute("ask", brandInfoService.getBrandInfoService(bm_no));
